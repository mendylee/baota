...
''' % (opt,);
    ...
    def GetMemcachedStatus(self,get):
        ...
        result = {}
        res = ['cmd_get','get_hits','get_misses','limit_maxbytes','curr_items','bytes','evictions','limit_maxbytes','bytes_written','bytes_read'];
        for d in data:
            if len(d)<3: continue;
            t = d.split();
            ...
            result[t[0]] = int(t[1]);
        result['hit'] = 0;
        if result['get_hits'] and result['cmd_get']:
            result['hit'] = result['get_hits'] / result['cmd_get'];
        return result;
