...
//获取负载
function getLoad(Average){
  $.get('/system?action=GetLoadAverage',function(data){
    ...
    if(Average <= data.safe){
      LoadColor = '#20a53a';/*流畅*/
      AverageText = '运行流畅';
      conterError = '服务器处在低负载状态中';
    }else if(data.safe  <= Average && Average <= data.max){
      LoadColor = '#6ea520';/*有卡顿*/
      AverageText = '运行正常';
      conterError = '服务器处在可承受的正常负载状态中';
    }else if(data.max <= Average && Average < data.limit){
      LoadColor = '#ff9900';/*有卡顿，运行慢*/
      AverageText = '运行缓慢';
      conterError = '服务器负载较高，运行缓慢，请考虑排查主要服务状态';
    }else if(data.limit <= Average){
      LoadColor = '#dd2f00';/*随时有宕机风险*/
      AverageText = '运行堵塞';
      conterError = '服务器已经抗不住了，随时有宕机的风险，请考虑故障转移，并立即排查原因';
    }
    setCookie('conterError',conterError);
    Occupy = Math.round((Average / data.limit) * 100);
    if(Occupy >= 100) Occupy = 100;
    if(Occupy <= 50){
      transformLeft = 0
      transformRight = parseInt(360 * (Occupy / 100));
    }else if( 50 < Occupy <= 100){
      transformLeft =  parseInt(360 * (Occupy - 50) / 100);
      transformRight =  parseInt(360 * 0.5);
    }
    index.find('.circle').css("background",LoadColor);
    index.find('.mask').css({"color":LoadColor});
    index.find('.left').css({"transform":"rotate("+ transformLeft +"deg)","background-color":"+ LoadColor+"});
    index.find('.right').css({"transform":"rotate("+ transformRight +"deg)","background-color":"+LoadColor+"});
    $('#Load').html(Occupy);
    $('#LoadState').html('<span style="color:'+ LoadColor +'">'+AverageText+'<a href="javascript:;" class="bt-ico-ask" target="_blank" title="" style="color:'+ LoadColor +';border-color:'+ LoadColor +';background-color: #fff !important;cursor: pointer;">?</a></span>');
    $('#LoadState .bt-ico-ask').hover(function(){
      var conterError = getCookie('conterError');
      layer.tips(conterError, '#LoadState span',{time:6000,tips:[1,'#999']});
    },function(){
        layer.closeAll('tips');
    });
  });
...
$('#LoadList .circle').hover(function(){
  	var one,five,fifteen;
  	one = getCookie('one');
 	five = getCookie('five');
 	fifteen = getCookie('fifteen');
   	var text = '最近1分钟平均负载：' + one + '</br>最近5分钟平均负载：'+ five +'</br>最近15分钟平均负载：'+ fifteen+ '</br>点击刷新负载状态';
    layer.tips(text, '#Load',{time:6000,tips:[1,'#999']});
},function(){
	layer.closeAll('tips');
});
	...
	$.get("/plugin?action=a&name=safelogin&s=GetServerInfo",function(rdata){
		$.get("/plugin?action=a&name=safelogin&s=get_ssh_errorlogin",function(tdata){
			...
			if(rdata.status){
				...
				GetWarning();
              	if(tdata.intrusion_total < 1000){
                  	var dangerhtmla='<p><img src="data:image/png;base64," style="margin-right:14px;vertical-align:-3px">升级企业运维版，每月只需98元，送双重安全隔离登录，每月送价值共534元的运维服务3次。<a class="btlink" style="margin-right: 5px;" href="javascript:SetSafeHide();">[暂时忽略] </a> <a class="btlink" href="https://www.bt.cn/admin/index.html" target="_blank">[立即升级]</a></p>';
					$("#messageError").append(dangerhtmla).show();
                }
